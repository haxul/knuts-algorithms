package heapSort

fun main() {
    val list = mutableListOf(2, 12, 23, 4, 10, 2, 100, 101,102)
    buildMaxHeap(list)
    print(list)
}

/** heap methods */

fun parent(i: Int): Int = i / 2
fun left(i: Int): Int = 2 * i
fun right(i: Int): Int = (2 * i) + 1

fun buildMaxHeap(A: MutableList<Int>) {
    val mid = A.size / 2
    for (i in mid downTo 0) {
        maxHeapify(A, i)
    }
}

fun maxHeapify(A: MutableList<Int>, i: Int) {
    val l = left(i)
    val r = right(i)
    var largest: Int

    largest = if (l < A.size && A[l] > A[i]) l else i // if left children is more than parent
    if (r < A.size && A[r] > A[largest]) largest = r
    if (largest != i) {
        A[i] = A[largest].also { A[largest] = A[i] }
        maxHeapify(A, largest)
    }
}